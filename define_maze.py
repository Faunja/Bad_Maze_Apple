...
class define_Maze:
	def check_maze(self):
		if self.position[0] != self.width - 1 or self.position[1] != self.height - 1:
			return True
		return False
	...
	def create_maze(self):
		while self.check_maze():
			if self.siding and self.position[1] != self.height - 1:
			...
			self.position[0] += 1
			if self.check_maze():
				if self.position[0] == self.width - 1 and self.siding:
					self.position[0] = 0
	...
	def __init__(self, width, height):
		...
		for row in range(self.height):
			...
			for column in range(self.width):
				...
				self.numbers[row].append(row * self.width + column)
		self.position = [0, 0]
